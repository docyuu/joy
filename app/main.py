...
app = FastAPI(title="Literature Workflow", version="0.3.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
class LLMConfig(BaseModel):
    enabled: bool = False
    provider: str = Field("deepseek", description="例如 deepseek / openai_compatible")
...
def fetch_pubmed(query: str, rows: int) -> list[dict[str, Any]]:
    ...
    id_csv = ",".join(ids)
    esummary = requests.get(PUBMED_ESUMMARY, params={"db": "pubmed", "id": id_csv, "retmode": "json"}, timeout=20)
...
def llm_summarize_paper(paper: Paper, llm: LLMConfig) -> str:
    if not llm.enabled:
        return paper.summary
    if not llm.api_key:
        raise HTTPException(status_code=400, detail="启用 LLM 时必须提供 api_key")

    endpoint = llm.base_url.rstrip("/") + "/v1/chat/completions"
    payload = {
        "model": llm.model,
        "messages": [
            {"role": "system", "content": "你是严谨的科研助手。"},
            {"role": "user", "content": build_llm_prompt(paper.title, paper.abstract, paper.methods)},
        ],
        "temperature": 0.3,
    }
    headers = {"Authorization": f"Bearer {llm.api_key}", "Content-Type": "application/json"}
    response = requests.post(endpoint, json=payload, headers=headers, timeout=llm.timeout_seconds)
    response.raise_for_status()
    content = extract_chat_content(response.json())
    return content or paper.summary


def apply_llm_summaries(papers: list[Paper], llm: LLMConfig | None) -> tuple[list[Paper], list[str]]:
...
def crawl(req: CrawlRequest) -> dict[str, Any]:
    ...
    papers, llm_errors = apply_llm_summaries(papers, req.llm)
    ...
    return {
        ...
        "llm_enabled": bool(req.llm and req.llm.enabled),
        "errors": errors,
